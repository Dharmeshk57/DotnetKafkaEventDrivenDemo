...
namespace Order.Consumer.Security;
...
///  Assuming SASL username "order-consumer" for this service:
///
///  Read from main and retry topics:
...
///      --group order-consumer-group.retry-1 \
///      --group order-consumer-group.retry-5 \
///      --group order-consumer-group.dlq \
///      --bootstrap-server broker:9093
///
///  Write to retry and DLQ topics (KafkaRouter):
///    kafka-acls.sh --add --allow-principal User:order-consumer \
///      --operation Write \
///      --topic order-events.retry-1 \
///      --topic order-events.retry-5 \
...
///      --bootstrap-server broker:9093
///
///  Principle of least privilege: the consumer cannot write to order-events,
///  and the producer (order-api) cannot read from any topic.
///
